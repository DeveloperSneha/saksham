        ...
        <table class="table table-bordered table-hover table-striped dataTable" id='table1'>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="{{url('admin/job/'.$var->idJobSession.'/edit')}}" target="_blank" style="text-decoration: none;display: inline-block;padding: 2px 10px;border-radius: 4px;background: #036101;color: #fff;cursor: pointer;font-size: 13px;"><i class="fa fa-eye"></i> Edit</a>
                        <button data-toggle="modal" data-target="#myModal1" data-id="{{$var->idJobSession}}" data-title="{{$var->sectorName}} {{$var->jobRoleName}}" class="video_pop intrested btn btn-xs btn-success"><i class="fa fa-check"></i> Activate</button>
                    </td>                    
                </tr>
                <?php $i++;?>
                @endforeach
...
<script>
    ...
    $(".explore_pop #link").attr('href', url+'/'+jobID+'/activated' );
    });
</script>
@stop