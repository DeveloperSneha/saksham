        ...
        <table class="table table-bordered table-hover table-striped dataTable" id='table1'>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{$var->toDate}}</td>
                    <td>
                        <button data-toggle="modal" data-target="#myModal1" data-id="{{$var->idJobSession}}" data-title="{{$var->job->jobrole->sector->sectorName}} {{$var->job->jobrole->jobRoleName}}" class="video_pop intrested btn btn-xs btn-danger"><i class="fa fa-close"></i> Deactivate</button>
                    </td>
                </tr>
                <?php $i++;?>
                @endforeach
