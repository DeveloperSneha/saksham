...
(function ( jQuery ) {
    ...
    jQuery.fn.startTreadmill = function( options ) {
        ...
        if(settings.viewable <= jQuery(this).children().length && settings.viewable != 0)
            treadmillHeight = settings.viewable * jQuery(this).find('.treadmill-unit').first().height();
        else
            treadmillHeight = jQuery(this).height();

        jQuery(this).css('height', treadmillHeight);
        ...
        if(settings.speed == "slow" || settings.speed == "medium" || settings.speed == "fast" || jQuery.isNumeric(settings.speed)) {
            ...
            jQuery.error("Super-Treadmill can only use one of its predefined speeds: 'slow', 'medium', 'fast' or a number that you specify, which it later converts into milliseconds");
        ...
        if(settings.pause) {
            ...
            jQuery(this).mouseleave( function() {
                running = !running;
                selectTreadmill(settings.direction, jQuery(this), treadSpeed);
            });
        }
